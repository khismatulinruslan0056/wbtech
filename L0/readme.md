...
Проект представляет собой микросервис на Go, который демонстрирует работу с очередью сообщений (Kafka), базой данных (PostgreSQL) и in-memory кэшированием. Сервис получает данные о заказах из Kafka, сохраняет их в базу данных и кэширует для быстрого доступа через HTTP API.
...
-   **Хранение**: Сохранение валидных данных в PostgreSQL.
